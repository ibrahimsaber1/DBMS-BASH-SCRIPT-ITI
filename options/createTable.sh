...
create_table () {
    ...
    # Check if the table name starts with an alphabetic character
        ...
        echo "general : Table Name : $table_name" > "$table_name"
        echo "general : Number of Columns : $colnumber" >> "$table_name"
        ...
        while [ $index -le $colnumber ]; do
            if [ $index -eq 1 ]; then
                ...
                echo "primary key : $index : $colname : int" >> "$table_name"
                printf "$colname:" >> "$table_name"
            else
                echo ""
                read -p "Enter column $index name: " colname
                echo ""
                ...
                echo "Not primary key : $index : $colname : $datatype" >> "$table_name"
                printf "$colname:" >> "$table_name"
            fi
            ((index = $index + 1))
        done
